...
test('fork preserves existing visual positions while adding only a new visual child', async ({ page }) => {
  ...
  expect(await nodes.nth(0).evaluate((el) => (el.parentElement as HTMLElement).style.transform))
    .toBe(rootBefore)
  expect(await nodes.nth(1).evaluate((el) => (el.parentElement as HTMLElement).style.transform))
    .toBe(childBefore)
})
