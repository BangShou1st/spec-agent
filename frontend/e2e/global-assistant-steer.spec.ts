...
test('scenario 2: late steer successor is observed after a pending-only activity read', async ({ page }) => {
  await mockProjects(page)
  await page.addInitScript((keys) => {
    window.localStorage.setItem(keys.thread, keys.threadId)
    window.localStorage.setItem(keys.run, keys.runId)
  }, { thread: 'spec-agent:global-assistant:thread:v1', run: 'spec-agent:global-assistant:run:v1', threadId: THREAD, runId: RUN_A })

  let activityReads = 0
  let handoff: 'pending' | 'successor' = 'pending'
  ...
  await page.route('**/api/v1/global-assistant/threads/' + THREAD + '/messages', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([{ id: 'm-1', threadId: THREAD, role: 'USER', content: 'first', runId: RUN_A, createdAt: '2026-09-10T00:00:00Z' }]) })
  })
  await page.route('**/api/v1/global-assistant/threads/' + THREAD + '/activity', async (route) => {
    if (route.request().method() !== 'GET') { await route.continue(); return }
    activityReads += 1
    const body = activityReads === 1
      ? { activeRun: { runId: RUN_A, status: 'RUNNING' }, pendingSteer: null }
      : handoff === 'pending'
        ? { activeRun: null, pendingSteer: { steerId: 's-1', message: '换方向', status: 'PENDING', interruptedRunId: RUN_A, successorRunId: null, createdAt: '2026-09-10T00:00:00Z' } }
        : { activeRun: { runId: RUN_B, status: 'RUNNING' }, pendingSteer: null }
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(body) })
  })
  await page.route('**/api/v1/global-assistant/runs/' + RUN_A + '/events', async (route) => {
    const frames = [{ sequence: 2, type: 'RUN_CANCELLED', payload: {}, runId: RUN_A }]
    if (acceptOf(route).includes('text/event-stream')) await route.fulfill({ status: 200, contentType: 'text/event-stream', body: sse(frames) })
    else await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([env(RUN_A, 1, 'RUN_STARTED')]) })
  })
  await page.route('**/api/v1/global-assistant/runs/' + RUN_B + '/events', async (route) => {
    const frames = [{ sequence: 1, type: 'STATUS', payload: { message: '正在处理新方向' }, runId: RUN_B }]
    if (acceptOf(route).includes('text/event-stream')) await route.fulfill({ status: 200, contentType: 'text/event-stream', body: sse(frames) })
    else await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(frames.map((f) => env(f.runId, f.sequence, f.type, f.payload))) })
  })
  await page.route('**/api/v1/global-assistant/runs/' + RUN_B, async (route) => {
  ...
  await openAssistant(page)
  // Backend legal intermediate state: A is terminal, no successor visible yet.
  await expect(page.getByTestId('ga-status')).toContainText('正在调整方向…')

  // The successor only becomes visible later: observation must pick it up.
  handoff = 'successor'
  await expect(page.getByTestId('ga-status')).toContainText('正在处理新方向')
  await expect(page.getByTestId('ga-steer-pending')).toHaveCount(0)
})
