  ...
  <div class="ga-tool" :class="stateClass" data-test="ga-tool-activity" :data-state="props.activity.state">
    ...
    >
      <span class="ga-tool__dot" aria-hidden="true">
        ...
        <span v-else-if="props.activity.state === 'success'">✓</span>
        <span v-else>!</span>
      </span>
      <span class="ga-tool__name">{{ props.activity.displayName }}</span>
      <span class="ga-tool__status">{{ stateLabel }}</span>
      <span class="ga-tool__chevron" aria-hidden="true">{{ expanded ? '▾' : '▸' }}</span>
    </button>
...
<style scoped>
.ga-tool { border: 1px solid var(--color-border); border-radius: 10px; background: var(--color-surface-subtle); margin: 6px 0; overflow: hidden; }
.ga-tool__head { width: 100%; display: flex; align-items: center; gap: 8px; padding: 8px 10px; background: transparent; border: 0; text-align: left; }
.ga-tool__head:focus-visible { outline: none; box-shadow: var(--focus-ring); border-radius: 10px; }
.ga-tool__dot { width: 20px; height: 20px; border-radius: 999px; display: inline-flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; border: 1px solid var(--color-border); background: var(--color-surface); color: var(--color-text-secondary); flex: none; }
...
.ga-tool__name { font-weight: 600; font-size: 13px; }
.ga-tool__status { margin-left: auto; font-size: 12px; color: var(--color-text-muted); }
.ga-tool__chevron { color: var(--color-text-muted); font-size: 12px; }
.ga-tool__summary { margin: 0 10px 8px 38px; font-size: 12.5px; color: var(--color-text-secondary); }
.ga-tool__detail { border-top: 1px dashed var(--color-border); margin: 0 10px 10px 38px; padding-top: 6px; }
