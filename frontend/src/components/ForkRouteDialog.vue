  ...
  <div v-if="open" class="dialog-backdrop" data-test="fork-dialog">
    <div class="dialog" role="dialog" aria-modal="true" aria-label="从此分支">
      ...
      <label class="secondary field-label">
        <span>路线名称（可选）</span>
        <input v-model="label" class="answer-input" data-test="fork-label" maxlength="255" placeholder="分支路线" />
      </label>
      <div class="dialog-actions">
        <button class="btn btn-primary" type="button" data-test="fork-submit" :disabled="!canSubmit" @click="submit">创建分支</button>
        <button class="btn" type="button" data-test="fork-cancel" :disabled="pending" @click="emit('close')">取消</button>
      </div>
    </div>
  </div>
</template>
