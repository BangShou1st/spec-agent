...
describe('model settings api', () => {
  it('uses the settings probe/save endpoints without exposing the key in reads', async () => {
    const get = vi.spyOn((await import('@/shared/http/client')).apiClient, 'get').mockResolvedValue({
      configured: false,
      maskedKey: null,
      selectedModel: null,
    })
    const post = vi.spyOn((await import('@/shared/http/client')).apiClient, 'post').mockResolvedValue({ freeModels: ['model-free'] })
    const put = vi.spyOn((await import('@/shared/http/client')).apiClient, 'put').mockResolvedValue({
      configured: true,
      maskedKey: '…1234',
