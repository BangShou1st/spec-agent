...
describe('answer draft cleanup identity', () => {
  ...
  it('answerSuccessClearsDraftForAnsweredNodeNotProducedNode', async () => {
    ...
    const { getProject } = await import('@/features/projects/api/projects')
    const { getRequirementState } = await import('@/features/workspace/api/requirementState')
    const { getActiveState, listRoutes } = await import('@/features/workspace/api/workspace')
    const { getProjectGraph } = await import('@/features/workspace/api/graph')
