...
import UiFormField from '@/shared/ui/UiFormField.vue'
import { managementErrorMessage } from '@/shared/http/errorCopy'
import type { ConnectionDetail } from '@/features/connections/api/connectionTypes'
import type { ConnectionsStoreError } from '@/features/connections/state/connectionsStore'
...
useDialogReset(toRef(props, 'open'), () => {
  ...
  const u = (props.detail as unknown as { config?: { serverUrl?: unknown } }).config?.serverUrl
...
<template>
  <UiDialogShell :open="open" title="编辑连接" description="仅改名可保持当前状态；修改地址或凭证后需要重新测试、连接并启用" test-id="connection-edit-dialog" @close="emit('close')">
    ...
    <ApiErrorBanner v-if="error" :message="managementErrorMessage(error.code, error.message)" data-test="edit-error" />
    <template #actions>
