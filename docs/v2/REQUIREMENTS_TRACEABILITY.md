...
| Question Node 可回答 | `NODE_MODEL_V2.md`, `UI_UX_IMPROVEMENT_PLAN.md` | Answer 仍为独立 immutable route-scoped record |
...
| Shared Node 要正确显示答案 | `GRAPH_MODEL_V2.md`, `UI_UX_IMPROVEMENT_PLAN.md` | Answer route-scoped；相同可合并摘要，不同按路线展示 |
| Focus Route 高亮但不独占 | `GRAPH_MODEL_V2.md`, `UI_UX_IMPROVEMENT_PLAN.md` | 其他路线降权但继续可见；Focus 不隐式 Activate |
| Active / Focus / Visibility 独立 | `GRAPH_MODEL_V2.md` | 保持现有冻结语义 |
| Route 使用友好名称 | `GRAPH_MODEL_V2.md`, `UI_UX_IMPROVEMENT_PLAN.md` | UI 不依赖 UUID/数据库编号 |
| Fork 后新路线立即出现 | `GRAPH_MODEL_V2.md`, `UI_UX_IMPROVEMENT_PLAN.md` | 先创建 Route，再运行 Agent；UI 投影 pending card |
