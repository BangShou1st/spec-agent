...
- the **first** projection of a `ContextSnapshot` into an `AgentInputSnapshot` is persisted once as a durable frozen projection (immutable payload + durable projection schema version `agent-input-projection.v2` + payload hash + mutable-source fingerprint set, insert-if-absent; the unique per-snapshot identity arbitrates concurrent first freezes — first writer wins, losers replay the winner's payload);
- the durable projection schema version `agent-input-projection.v2` is **independent** of the wire envelope version `agent-input.v2` (see `contracts/README.md`): the frozen `payload` happens to be the canonical serialization of the `snapshot` field of an `agent-input.v2` envelope, but the durable table's version evolves separately. Legacy rows shipped as `agent-input-projection.v1` or `agent-input.v2` remain readable; any other version fails closed;
