 ...
 *
 * <p>The empty-state tests must own their credential state: a leftover OpenCode
 * credential from a live smoke or manual seed in the local dev/test database
 * must not change their outcome, so every test starts from a clean
 * {@code provider_credentials} row for the OpenCode provider.
 */
@SpringBootTest
@ActiveProfiles("test")
@Transactional
